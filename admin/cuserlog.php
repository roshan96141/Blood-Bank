        ...
        <form action="" method="post">
            ...
            <div class="form1">
                <button type="submit" name="login" class="button">Login</button>
                 <strong class="white">New User?<a href="campregis.php" class="a">Register</a></strong>
            </div>
                </form>
            <?php
            ...
            $un=$_POST['uname'];
            $ps=$_POST['password1'];
            $a=$db->prepare("SELECT * FROM cregis where cname='$un' && pass='$ps'");
            $a->execute();
            $res=$a->fetchAll(PDO::FETCH_OBJ);
            if($res)
            {
                $_SESSION['uname']=$un;
                header("location:camphome.php");
            }else
            {
